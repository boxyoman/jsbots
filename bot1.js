...
function main(){
	if(gameTime.cTime > 3){
		...
		if(-sign(vel.x) == sign(Math.cos(ang))){ //Both Kinetic force and applied force are acting in the same direction
			bot.force.x = Math.cos(ang)*velMag*velMag/a.mag()-sign(Math.cos(ang));
		}else{
			bot.force.x = Math.cos(ang)*velMag*velMag/a.mag()+sign(Math.cos(ang));
		}
		if(-sign(vel.y) == sign(Math.sin(ang))){ //Both Kinetic force and applied force are acting in the same direction
